# =============================================================================
# Zsh Configuration for st4rk
#
# This file configures the environment for login and non-interactive shells.
# For interactive sessions, it executes and hands off to the fish shell.
# =============================================================================

# --- Execute Fish for Interactive Shells -------------------------------------
# If this is an interactive shell, execute fish.
if [ -z "$FISH_VERSION" ] && [ -n "$PS1" ] && [ -x "$(command -v fish)" ]; then
  exec fish
fi
...
# Nix environment
if [ -e /home/st4rk/.nix-profile/etc/profile.d/nix.sh ]; then
  . /home/st4rk/.nix-profile/etc/profile.d/nix.sh; # added by Nix installer
fi
...
if [ $(echo $MANPAGER | awk '{print $1}') = nvim ]; then
  ...
  # The original file set MANPAGER to less here, overriding the nvim setting above.
  # If you want to use nvim as manpager, you should remove the next line.
  # If you want to use less with colors, keep it.
...
export LESSHISTFILE=-
# HIST_STAMPS=yyyy/mm/dd # Timestamps disabled
