...
const char *spcmd2[] = {
    "st",   "-n",      "spcalc",        "-g", "120x28", "-e",
    "qalc", "--color", "--interactive", NULL};
const char *spcmd3[] = {"qalculate-gtk", NULL};
const char *spcmd4[] = {"st",     "-n", "spmusic", "-g",
                        "100x25", "-e", "cmus",    NULL};
const char *spcmd5[] = {"st",     "-n", "spnotes", "-g",
                        "120x34", "-e", "spnotes", NULL};
static Sp scratchpads[] = {
    /* name          cmd  */
    {"spterm", spcmd1},  {"spcalc", spcmd2},  {"spcalcgui", spcmd3},
    {"spmusic", spcmd4}, {"spnotes", spcmd5},
};
...
static const Rule rules[] = {
    ...
    RULE(.wintype = WTYPE "DIALOG", .isfloating = 1) RULE(
            ...
            .isfloating = 1) RULE(.wintype = WTYPE "TOOLBAR",
                                  .isfloating = 1) RULE(.wintype =
                                                            WTYPE "SPLASH",
                                                        .isfloating = 1)

        RULE(.class = "firefox", .tags = 1 << 1, .switchtag = 1) RULE(
                .title = "nvim", .tags = 1 << 2,
                .switchtag = 1) RULE(.title = "lf", .tags = 1 << 3,
                                     .switchtag = 1) RULE(.class = "mpv",
                                                          .tags = 1 << 4,
                                                          .switchtag = 3)
            RULE(.title = "newsboat", .tags = 1 << 5,
                 .switchtag = 3) RULE(.class = "Zathura", .tags = 1 << 6,
                                      .switchtag = 3)
                RULE(.title = "nsxiv", .tags = 1 << 7,
                     .switchtag = 3) RULE(.class = "Gimp", .tags = 1 << 8,
                                          .switchtag = 3)
                    RULE(.instance = "libreoffice", .tags = 1 << 8,
                         .switchtag = 3)

                        RULE(.instance = "spterm", .tags = SPTAG(0),
                             .isfloating = 1)
                            RULE(.instance = "spcalc", .tags = SPTAG(1),
                                 .isfloating = 1) RULE(.class = "Qalculate-gtk",
                                                       .tags =
                                                           SPTAG(2),
                                                       .isfloating = 1)
                                RULE(.instance = "spmusic", .tags = SPTAG(3),
                                    ...
                                    RULE(.instance = "dictionary",
                                         .isfloating = 1) RULE(.class =
                                                                   "volume-ui",
                                                               .isfloating = 1)
                                        RULE(.class = "gping-ui",
                                             .isfloating = 1)
                                            RULE(.class = "PersepolisDM",
                                                 .title = "(None)None",
                                                 .isfloating = 1)
...
// 	NULL
// };
static const char *dmenu_run_cmd[] = {"dmenu_run", "-bw", "2",   "-i", "-W",
                                      "390",       "-X",  "961", "-Y", "15",
                                      "-l",        "15",  "-g",  "3",  NULL};
static const char *clipmenu_cmd[] = {"clipmenu", "-bw", "2",    "-i", "-W",
                                     "290",      "-X",  "1061", "-Y", "15",
                                     "-l",       "15",  NULL};
static const char *volume_ui_cmd[] = {
    "st", "-c", "volume-ui", "-g=80x15+353+20", "-e", "pulsemixer", NULL};
...
static const Key on_empty_keys[] = {
    ...
    {0,
     XK_Return,
     spawn,
     {.v = (const char *[]){"st", "-c", "st-terminal", NULL}}},
    ...
    {0,
     XK_p,
     spawn,
     {.v = (const char *[]){"st", "-c", "gping-ui", "-g=80x15+353+20", "-e",
                            "gping", "duckduckgo.com", NULL}}},
 ...
 */
    ...
    {0, XF86XK_AudioMute, spawn,
     SHCMD("wpctl set-mute @DEFAULT_AUDIO_SINK@ toggle; pkill -RTMIN+8 "
           "dwmblocks")},
    {0, XF86XK_AudioRaiseVolume, spawn,
     SHCMD("wpctl set-volume @DEFAULT_AUDIO_SINK@ 0%- && wpctl set-volume "
           "@DEFAULT_AUDIO_SINK@ 3%+; pkill -RTMIN+8 dwmblocks")},
    ...
    // { ALTKEY,             XK_Return,                spawn,                {.v
    // = (const char *[]){"kitty", "-d", "~/projects/videos/fzf", NULL}}},
    {ALTKEY, XK_Return, spawn, {.v = (const char *[]){"kitty", NULL}}},
    {MODKEY, XK_v, spawn, {.v = (const char *[]){"st", "men", NULL}}},
    {MODKEY | ShiftMask,
     XK_b,
     spawn,
     {.v = (const char *[]){"dmenu_web", "--add", NULL}}},
    ...
    {MODKEY,
     ...
     spawn,
     {.v =
          (const char *[]){"mpv", "--player-operation-mode=pseudo-gui", NULL}}},
    {MODKEY | ShiftMask,
     XK_v,
     spawn,
     {.v = (const char *[]){"jellyfinmediaplayer", NULL}}},
    ...
    {ALTKEY, XK_comma, togglescratch, {.ui = 4}},
#if RIODRAW_PATCH
...
static const Button buttons[] = {
    ...
    {ClkRootWin, 0, Button3, spawn,
     SHCMD("dunstctl close-all; pgrep -x 'dmenu' > /dev/null && killall dmenu "
           "|| dmenu_hub")},
#if BAR_STATUSBUTTON_PATCH
